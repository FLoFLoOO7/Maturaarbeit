...
moves = {
    ...
    'D':  [kippen,normal,vorne,kippen,vorne,normal,kippen,vorne,kippen,normal,vorne,kippen,vorne],
    'Dr': [kippen,normal,vorne,kippen,vorne,uhrzeigersinn,kippen,vorne,kippen,normal,vorne,kippen,vorne],
    'D2':[kippen,normal,vorne,kippen,vorne,doppel,kippen,vorne,kippen,normal,vorne,kippen,vorne],
...
def OptimizeMoves(move_list):
    ...
    print(optimized)
    return optimized
def DoMoves(move_list):
    move_list=OptimizeMoves(move_list)
...
def EckenF2l(x,executed):
    ...
    while counter < 4:
        ...
        for move in moves:
            x=move_map[move](x)
            Ziel = [(NewTuple(a, move), NewTuple(b, move),NewTuple(c,move)) for (a, b, c) in Ziel]
...
def FindeKantenV4(a):
    ...
    for i in range(1,6):
        for l in kanten:
            ...
            if a[i][l] == a[0][4]:
                continue
            if a[kanten_f[p][0]][kanten_f[p][1]] == a[2][4]:   
                continue
...
def KantenF2l(x,executed):
    
    counter = 0

    kanten = FindeKantenV4(x)  
    farben = [x[1][4], x[5][4], x[4][4], x[3][4]]
    move_fn = {
    ...
    while counter < 4:
        ...
        for move in moves:
            ...
            Ziel = [(NewTuple(a, move), NewTuple(b, move)) for (a, b) in Ziel]
        counter += 1        
    print(executed,x)
    return x,executed  

#OLL
def Oll(Form,executed):
    executed=[]
...
def PllV2(x,executed ):
    ...
    while True:
        ...
        final_kanten=[None]*4
        targets=[x[1][4],x[5][4],x[3][4],x[4][4]]
        for idx, color in enumerate(targets):
            if color in kanten:
                final_kanten[idx]=(kanten.index(color), idx)
        final_kanten = tuple(final_kanten)

        if final_kanten in PLL_kanten:
...
def ausfuhren():
    cube_faces=colorsensor()
    x=converter(cube_faces)
    x=[['Gre', 'Gel', 'Wei', 'Gre', 'Gre', 'Gre', 'Wei', 'Wei', 'Wei'], 
...
['Ora', 'Wei', 'Rot', 'Blu', 'Gel', 'Wei', 'Blu', 'Gel', 'Blu']]
    x,executed=KreuzV5(x)
    Form,executed=EckenF2l(x,executed)
    x,executed=KantenF2l(Form,executed)
    Form,executed=Oll(x,executed)
    final=PllV2(Form,executed)
    DoMoves(final)
ausfuhren()
